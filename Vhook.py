...
\$$\  $$  |$$  __$$\ $$  __$$\ $$  __$$\ $$ | $$  |
 \$$\$$  / $$ |  $$ |$$ /  $$ |$$ /  $$ |$$$$$$  / 
  \$$$  /  $$ |  $$ |$$ |  $$ |$$ |  $$ |$$  _$$<  
   \$  /   $$ |  $$ |\$$$$$$  |\$$$$$$  |$$ | \$$\ 
    \_/    \__|  \__| \______/  \______/ \__|  \__|

    vesper#0003
    @i_might_be_vesper
    Also, skidding wont make you a coder ;)
"""
import os
import re
import win32gui
...
window.iconbitmap("assets/vhook.ico")
...
Also, skidding wont make you a coder ;)
        """)
        print(Fore.YELLOW+"["+Fore.WHITE+"+"+Fore.YELLOW+"]"+Fore.WHITE+" LOGS")
    ...
    def justsendwebh(self):
        ...
        try:
            ...
            messagebox.showerror('Vhook', 'Invalid Amount')
            self.__init__()
        ...
        if self.delwbhison == False:
            for i in range(amount):
                ...
                if r.status_code ==404:
                    messagebox.showerror('Vhook', 'Invalid Webhook')
                    self.__init__()
                    hellobitch = True
                    break
            ...
            if hellobitch == True:
                ...
                messagebox.showinfo('Vhook', 'Successfully Spammed The Webhook !')
                self.__init__()
        else:
            for i in range(amount):
                r = requests.post(webhook, json={'content' : f'{content}'})
            ...
            if hellobitch == True:
                ...
                messagebox.showinfo('Vhook', 'Successfully Deleted And Spammed The Webhook!')
    ...
    def checkdelorfuckoff(self):
        if self.delwbhison == True:
            ...
            messagebox.showinfo('Vhook', 'Successfully Deleted The Webhook!')
            self.__init__()
        else:
            messagebox.showinfo('Vhook', 'No action have been selected')
            self.__init__()
        ...
        if poop_lol == True:
            ...
            while True:
                ...
                if len(content) > 0:
                    ...
                    itworkslol = False
                    messagebox.showerror('Vhook', 'Fill Out Content !')
                    break
                try:
                    amount = int(self.dadvamount.get())
                    if amount > 0:
                ...
                if len(username) > 0:
                    ...
                    itworkslol = False
                    messagebox.showerror('Vhook', 'Fill Out Username !')
                    break
                avatar_url = self.dadvavatarurl.get()
                if len(avatar_url) > 0:
                    if avatar_url.startswith('https://cdn.discordapp.com/'):
                        ...
                        itworkslol = False
                        messagebox.showerror('Vhook', 'Invalid cnd Attachement For Avatar Url')
                        break
                else:
                    itworkslol = False
                    messagebox.showerror('Vhook', 'Fill Out Avatar_Url !')
                    break
                title = self.dadvtitle.get()
                if len(title) > 0:
                    self.embedon +=1
                    ...
                    itworkslol = False
                    messagebox.showerror('Vhook', 'Fill Out Title !')
                    break
                desc = self.dadvdesc.get()
                if len(desc) > 0:
                    self.embedon +=1
                    ...
                    itworkslol = False
                    messagebox.showerror('Vhook', 'Fill Out Description !')
                    break
                ...
                if color != None:
                    ...
                    itworkslol = False
                    messagebox.showerror('Vhook', 'Invalid Color')
                    break
                author = self.dadvauthor.get()
                if len(author) > 0:
                    self.embedon +=1
                    ...
                    itworkslol = False
                    messagebox.showerror('Vhook', 'Fill Out Author !')
                    break
                footer = self.dadvfooter.get()
                if len(footer) > 0:
                    self.embedon +=1
                    ...
                    itworkslol = False
                    messagebox.showerror('Vhook', 'Fill Out Footer !')
                    break
                gif = self.dadvgif.get()
                if len(gif) > 0:
                    self.embedon +=1
                    if gif.startswith('https://cdn.discordapp.com/'):
                        ...
                        itworkslol = False
                        messagebox.showerror('Vhook', 'Invalid')
                        break
                else:
                    itworkslol = False
                    messagebox.showerror('Vhook', 'Invalid cnd Attachement for GIF / Image')
                    break
                if itworkslol == True:
                    break
            if itworkslol == True:
    ...
    def webhookspamndeletemain(self):
        ...
        bkkk.place(x=0,y=0)
        self.dadvwebhook = Entry(window,text='a',font=('SeoulHangang',10),bg='#A7A7A7', fg='#BF7B3D',width=46,borderwidth=0,show='*')
        self.dadvwebhook.place(x=214, y=138)
        self.dadvcontent = Entry(window,text='b',font=('SeoulHangang',10),bg='#A7A7A7', fg='#BF7B3D',width=46,borderwidth=0)
        self.dadvcontent.place(x=214, y=173)
        self.dadvamount = Entry(window,text='c',font=('SeoulHangang',10),bg='#A7A7A7', fg='#BF7B3D',width=5,borderwidth=0)
        self.dadvamount.place(x=214, y=209)
        self.dadvusername = Entry(window,text='d',font=('SeoulHangang',10),bg='#A7A7A7', fg='#BF7B3D',width=19,borderwidth=0)
        self.dadvusername.place(x=192, y=290)
        self.dadvavatarurl = Entry(window,text='e',font=('SeoulHangang',10),bg='#A7A7A7', fg='#BF7B3D',width=19,borderwidth=0)
        self.dadvavatarurl.place(x=192, y=321)
        self.dadvtitle = Entry(window,text='f',font=('SeoulHangang',10),bg='#A7A7A7', fg='#BF7B3D',width=19,borderwidth=0)
        self.dadvtitle.place(x=192, y=352)
        self.dadvdesc = Entry(window,text='g',font=('SeoulHangang',10),bg='#A7A7A7', fg='#BF7B3D',width=19,borderwidth=0)
        self.dadvdesc.place(x=192, y=383)
        self.dadvauthor = Entry(window,text='h',font=('SeoulHangang',10),bg='#A7A7A7', fg='#BF7B3D',width=19,borderwidth=0)
        self.dadvauthor.place(x=459, y=321)
        self.dadvfooter = Entry(window,text='i',font=('SeoulHangang',10),bg='#A7A7A7', fg='#BF7B3D',width=19,borderwidth=0)
        self.dadvfooter.place(x=459, y=352)
        self.dadvgif = Entry(window,text='j',font=('SeoulHangang',10),bg='#A7A7A7', fg='#BF7B3D',width=19,borderwidth=0)
